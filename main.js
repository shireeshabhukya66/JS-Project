...
//     }
//     if (password !== cmpassword) { alert("Passwords do not match"); return; }
//     if (!/^\d{10}$/.test(number)) { alert("Phone number must be 10 digits"); return; }
...
//     alert("Signup successful!");
//     inputForm.reset();
//     document.querySelector("#loginEmail").value = email; // auto-fill login email
//     registerForm.style.display = "block";
//     inputForm.style.display = "none";
// });
...
// loginBtnFinal.addEventListener("click", function (e) {
//     e.preventDefault();
//     const loginEmail = document.querySelector("#registerForm input[type='email']").value.trim();
//     const loginPassword = document.querySelector("#registerForm input[type='password']").value;

//     let users = JSON.parse(localStorage.getItem("users")) || [];
//     const validUser = users.find(u => u.email === loginEmail && u.password === loginPassword);

//     if (!validUser) { alert("Invalid email or password!"); return; }
